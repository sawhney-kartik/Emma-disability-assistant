  ...
  $scope.criteriaMatch = function(criteria) {
    return function(item) {
      ...
      for (var i = 0; i < criteriaArrayTemp.length; i++) {
        if (stopWords.indexOf(criteriaArrayTemp[i].toLowerCase()) > -1) {
          continue;
        }
        criteriaArray.push(criteriaArrayTemp[i].toLowerCase());
      }

      for (var j = 0; j < itemArrayTemp.length; j++) {
        if (stopWords.indexOf(itemArrayTemp[j].toLowerCase()) > -1) {
          continue;
        }
        itemArray.push(itemArrayTemp[j].toLowerCase());
      }

      for (var k = 0; k < itemArrayTempSecond.length; k++) {
        if (stopWords.indexOf(itemArrayTempSecond[k].toLowerCase()) > -1) {
          continue;
        }
        itemArray.push(itemArrayTempSecond[k].toLowerCase());
      }

      for (var l = 0; l < itemArrayTempThird.length; l++) {
        if (stopWords.indexOf(itemArrayTempThird[l].toLowerCase()) > -1) {
          continue;
        }
        itemArray.push(itemArrayTempThird[l].toLowerCase());
      }
